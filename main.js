...
(function(){
  ...
  function extractMainContent(html){ 
    try{ 
      ...
      if(!main) return null; 
      return {innerHTML:main.innerHTML,title:doc.title}; 
    }catch(e){return null;} 
  }
  function replaceMainContent(htmlObj){ 
    const c=document.getElementById(MAIN_CONTENT_ID); 
    if(!c) return false; 
    c.innerHTML=htmlObj.innerHTML; 
    if(htmlObj.title) document.title=htmlObj.title; 
    window.scrollTo(0,0); 
    return true; 
  }

  // ✅ Global navigateTo function for footer & SPA
  function navigateTo(path,push=true){ 
    const key=normalizeUrl(path); 
    if(cache[key]){
      replaceMainContent(cache[key]); 
      if(push) history.pushState({url:key},'',key);
    } else { 
  ...
  window.pagesToPreload = pagesToPreload;
  ...
  document.addEventListener('click',function(e){ 
    ...
    if(pagesToPreload.map(normalizeUrl).includes(norm)){
      e.preventDefault();
      navigateTo(norm,true); 
    }
  });
  ...
  function preloadSingle(url){ 
    ...
    fetch(url,{credentials:'same-origin'})
      ...
      .then(html=>{
        ...
        if(main) cache[key]=main;
      })
      .catch(()=>{}); 
  }
  function preloadAll(){ pagesToPreload.map(preloadSingle); }
  window.addEventListener('popstate',function(e){ 
    const url=(e.state&&e.state.url)?e.state.url:currentPath(); 
    if(cache[url]) replaceMainContent(cache[url]); 
